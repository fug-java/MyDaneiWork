...
-- 05.重新创建mydb1库，指定编码为utf8
	...
	-- 如果后面没有指定编码, 将会使用默认的编码(iso8859-1), 因此一定要指定编码!!
...
-- 10.往学生表(stu)中插入记录(数据)
	...
	insert into stu(id,name,gender,birthday,score) values(null,'wht','1','1988-6-28',78);
	insert into stu values(null,'lpx','1','1978-4-30',88);
	insert into stu values(null,'陈子枢','男','1974-6-24',83);
	/* 提示：
	 设置编码：set names gbk;
...
-- 16.查询emp表中的所有员工，显示所有列
	select id,name,gender,birthday,dept,salary,bonus from emp;
	select * from emp;
...
-- 19.查询emp表中总薪资(薪资+奖金)大于3500的所有员工，显示员工姓名、总薪资
	...
	-- as用于指定别名,可以省略
	select name 姓名, sal+ifnull(bonus,0) 总薪资
	from emp
	where sal+ifnull(bonus,0) > 3500;
...
-- 21.查询emp表中薪资为1400、1600、1800的员工，显示员工姓名和薪资
	select name,sal from emp
...
-- 31.对emp表按照部门进行分组, 求每个部门的最高薪资(不包含奖金)，显示部门名称和最高薪资
	...
	-- 求每个部门的最高薪资
	select dept, name, max(sal) from emp 
	group by dept;
...
-- 准备数据： 以下练习将使用db30库中的表及表记录，请先进入db30数据库!!!
-- 43.查询部门和部门对应的员工信息
	select * from dept,emp; -- 会有大量错误的数据
